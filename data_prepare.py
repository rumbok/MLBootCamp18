...
def cons_kpi(csi_df, cons_df, kpi_df, name: str):
    if os.path.isfile(os.path.join(CACHE_DIR, f'{name}.feather')):
        ...
        kpi_df['T_DATE'] = pd.to_datetime(kpi_df['T_DATE'])
        kpi_df['MON'] = kpi_df['T_DATE'].dt.to_period('M')

        cons_df = pd.merge(cons_df, kpi_df.groupby(by=['CELL_LAC_ID', 'MON']).mean(),
                           on=['CELL_LAC_ID', 'MON'],
                           how='left',
                           suffixes=('', f'_mean'))
        ...
        cons_df[cons_df.select_dtypes(include=['datetime64']).columns] = \
            cons_df[cons_df.select_dtypes(include=['datetime64']).columns].astype(str)
...
def merge_all(df, feat_df, cons_df, data_avg_df, data_chnn_df, voice_avg_df, voice_chnn_df):
...
if __name__ == '__main__':
    ...
    train_df = load_csi_train()
    res_df = add_consumption(train_df, cons_df)
